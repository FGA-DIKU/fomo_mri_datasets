...
def main(source_path, dest_path, num_workers=12):
    subjects_dir = join(source_path, "ds004146")
    ...
    for subject in dirs_in_dir(subjects_dir):
        subjects += 1
        sessions_dir = join(subjects_dir, subject)
        for session in dirs_in_dir(sessions_dir):
            sessions += 1
            modalities_dir = join(sessions_dir, session)
            for modality in dirs_in_dir(modalities_dir):
                if modality in ["anat", "swi"]:
                    scans_dir = join(modalities_dir, modality)
                    for filename in subfiles(scans_dir, join=False, suffix=ext):
                        if "defacemask" in filename.lower():
                            continue
                        tasks.append(
                            {
                                "filename": filename,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Process data from QTAB dataset")
    parser.add_argument(
        "--input",
        type=str,
        required=True,
        help="Path to QTAB data directory"
    )
    parser.add_argument(
        "--output",
        type=str,
