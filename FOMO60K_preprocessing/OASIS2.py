...
def main(source_path, dest_path, num_workers=12):
    ...
    for subject in tqdm(all_subjects, desc="Subjects"):
        # Expecting folder name like: OAS2_0001_MR1
        ...
        for filename in nii_files:
            ...
            # Extract the part after the last dash (e.g. from "mpr-1", extract "1")
            t1_number = scan_base.split('-')[-1]
            out_filename = f"T1_{t1_number}.nii.gz"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Process data from OASIS2 dataset")
    parser.add_argument(
        "--input",
        type=str,
        required=True,
        help="Path to OASIS2 data directory"
    )
    parser.add_argument(
        "--output",
        type=str,
