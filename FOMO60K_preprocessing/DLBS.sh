...
usage() {
    ...
    echo ""
    echo "Process DLBS dataset with preprocessing pipeline"
    echo ""
    echo "Options:"
    echo "  -i: Input root directory containing DLBS subject/session folders"
    echo "  -o: Output root directory for preprocessed DLBS data"
    echo "  -t: Number of threads (default: 4)"
    echo "  -f: FreeSurfer home directory (default: \$FREESURFER_HOME)"
    echo "  -h: Show this help message"
    echo ""
    echo "Example:"
    echo "  $0 -i /path/to/dlbs_tmp -o /path/to/dlbs_preprocessed -t 4"
    exit 1
}
...
# Set up FreeSurfer
echo "=================================================="
echo "DLBS Dataset Preprocessing Pipeline"
echo "=================================================="
...
# Get all DLBS session directories (2 levels deep: subject/session)
echo "Scanning for DLBS sessions in: $INPUT_ROOT"
SESSIONS=($(find "$INPUT_ROOT" -mindepth 2 -maxdepth 2 -type d | sort))

# Check if we have sessions
if [ ${#SESSIONS[@]} -eq 0 ]; then
    echo "Error: No DLBS sessions found in $INPUT_ROOT"
    echo "Expected structure: $INPUT_ROOT/subject_*/session_*/"
    exit 1
fi

echo "Found ${#SESSIONS[@]} DLBS sessions to process"
...
for SESSION_DIR in "${SESSIONS[@]}"; do
    echo "Processing DLBS session: $SESSION_DIR"
