...
usage() {
    ...
    echo ""
    echo "Process SOOP dataset with preprocessing pipeline"
    echo ""
    echo "Options:"
    echo "  -i: Input root directory containing SOOP subject/session folders"
    echo "  -o: Output root directory for preprocessed SOOP data"
    echo "  -t: Number of threads (default: 4)"
    echo "  -f: FreeSurfer home directory (default: \$FREESURFER_HOME)"
    echo "  -h: Show this help message"
    echo ""
    echo "Example:"
    echo "  $0 -i /path/to/soop_tmp -o /path/to/soop_preprocessed -t 4"
    exit 1
}
...
# Set up FreeSurfer
echo "=================================================="
echo "SOOP Dataset Preprocessing Pipeline"
echo "=================================================="
...
# Get all SOOP session directories (2 levels deep: subject/session)
echo "Scanning for SOOP sessions in: $INPUT_ROOT"
SESSIONS=($(find "$INPUT_ROOT" -mindepth 2 -maxdepth 2 -type d | sort))

# Check if we have sessions
if [ ${#SESSIONS[@]} -eq 0 ]; then
    echo "Error: No SOOP sessions found in $INPUT_ROOT"
    echo "Expected structure: $INPUT_ROOT/subject_*/session_*/"
    exit 1
fi

echo "Found ${#SESSIONS[@]} SOOP sessions to process"
...
for SESSION_DIR in "${SESSIONS[@]}"; do
    echo "Processing SOOP session: $SESSION_DIR"
