...
usage() {
    ...
    echo ""
    echo "Process OASIS2 dataset with preprocessing pipeline"
    echo ""
    echo "Options:"
    echo "  -i: Input root directory containing OASIS2 subject/session folders"
    echo "  -o: Output root directory for preprocessed OASIS2 data"
    echo "  -t: Number of threads (default: 4)"
    echo "  -f: FreeSurfer home directory (default: \$FREESURFER_HOME)"
    echo "  -h: Show this help message"
    echo ""
    echo "Example:"
    echo "  $0 -i /path/to/oasis2_tmp -o /path/to/oasis2_preprocessed -t 4"
    exit 1
}
...
# Set up FreeSurfer
echo "=================================================="
echo "OASIS2 Dataset Preprocessing Pipeline"
echo "=================================================="
...
# Get all OASIS2 session directories (2 levels deep: subject/session)
echo "Scanning for OASIS2 sessions in: $INPUT_ROOT"
SESSIONS=($(find "$INPUT_ROOT" -mindepth 2 -maxdepth 2 -type d | sort))

# Check if we have sessions
if [ ${#SESSIONS[@]} -eq 0 ]; then
    echo "Error: No OASIS2 sessions found in $INPUT_ROOT"
    echo "Expected structure: $INPUT_ROOT/subject_*/session_*/"
    exit 1
fi

echo "Found ${#SESSIONS[@]} OASIS2 sessions to process"
...
for SESSION_DIR in "${SESSIONS[@]}"; do
    echo "Processing OASIS2 session: $SESSION_DIR"
    ...
    if bash "$PREPROCESS_SCRIPT" "$SESSION_DIR" "$OUTPUT_DIR" --extension ".$EXT" --threads $THREADS; then
        echo "  → Success"
        ((PROCESSED++))
    else
        echo "  → Failed"
...
# Print summary
echo "=================================================="
echo "OASIS2 Processing Complete!"
echo "=================================================="
