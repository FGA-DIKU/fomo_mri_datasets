...
usage() {
    ...
    echo ""
    echo "Process MSD BrainTumor dataset with preprocessing pipeline"
    echo ""
    echo "Options:"
    echo "  -i: Input root directory containing MSD BrainTumor subject/session folders"
    echo "  -o: Output root directory for preprocessed MSD BrainTumor data"
    echo "  -t: Number of threads (default: 4)"
    echo "  -f: FreeSurfer home directory (default: \$FREESURFER_HOME)"
    echo "  -h: Show this help message"
    echo ""
    echo "Example:"
    echo "  $0 -i /path/to/msd_braintumor_tmp -o /path/to/msd_braintumor_preprocessed -t 4"
    exit 1
}
...
# Set up FreeSurfer
echo "=================================================="
echo "MSD BrainTumor Dataset Preprocessing Pipeline"
echo "=================================================="
...
# Get all MGH Wild session directories (2 levels deep: subject/session)
echo "Scanning for MSD BrainTumor sessions in: $INPUT_ROOT"
SESSIONS=($(find "$INPUT_ROOT" -mindepth 2 -maxdepth 2 -type d | sort))

# Check if we have sessions
if [ ${#SESSIONS[@]} -eq 0 ]; then
    echo "Error: No MSD BrainTumor sessions found in $INPUT_ROOT"
    echo "Expected structure: $INPUT_ROOT/subject_*/session_*/"
    exit 1
fi

echo "Found ${#SESSIONS[@]} MSD BrainTumor sessions to process"
...
for SESSION_DIR in "${SESSIONS[@]}"; do
    echo "Processing MSD BrainTumor session: $SESSION_DIR"
...
# Print summary
echo "=================================================="
echo "MSD BrainTumor Processing Complete!"
echo "=================================================="
