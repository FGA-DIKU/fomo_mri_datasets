...
usage() {
    ...
    echo ""
    echo "Process MBSR dataset with preprocessing pipeline"
    echo ""
    echo "Options:"
    echo "  -i: Input root directory containing MBSR subject/session folders"
    echo "  -o: Output root directory for preprocessed MBSR data"
    echo "  -t: Number of threads (default: 4)"
    echo "  -f: FreeSurfer home directory (default: \$FREESURFER_HOME)"
    echo "  -h: Show this help message"
    echo ""
    echo "Example:"
    echo "  $0 -i /path/to/mbsr_tmp -o /path/to/mbsr_preprocessed -t 4"
    exit 1
}
...
# Set up FreeSurfer
echo "=================================================="
echo "MBSR Dataset Preprocessing Pipeline"
echo "=================================================="
...
# Get all MBSR session directories (2 levels deep: subject/session)
echo "Scanning for MBSR sessions in: $INPUT_ROOT"
SESSIONS=($(find "$INPUT_ROOT" -mindepth 2 -maxdepth 2 -type d | sort))

# Check if we have sessions
if [ ${#SESSIONS[@]} -eq 0 ]; then
    echo "Error: No MBSR sessions found in $INPUT_ROOT"
    echo "Expected structure: $INPUT_ROOT/subject_*/session_*/"
    exit 1
fi

echo "Found ${#SESSIONS[@]} MBSR sessions to process"
...
for SESSION_DIR in "${SESSIONS[@]}"; do
    echo "Processing MBSR session: $SESSION_DIR"
