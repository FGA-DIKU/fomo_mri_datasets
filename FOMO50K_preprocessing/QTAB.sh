...
usage() {
    ...
    echo ""
    echo "Process QTAB dataset with preprocessing pipeline"
    echo ""
    echo "Options:"
    echo "  -i: Input root directory containing QTAB subject/session folders"
    echo "  -o: Output root directory for preprocessed QTAB data"
    echo "  -t: Number of threads (default: 4)"
    echo "  -f: FreeSurfer home directory (default: \$FREESURFER_HOME)"
    echo "  -h: Show this help message"
    echo ""
    echo "Example:"
    echo "  $0 -i /path/to/qtab_tmp -o /path/to/qtab_preprocessed -t 4"
    exit 1
}
...
# Set up FreeSurfer
echo "=================================================="
echo "QTAB Dataset Preprocessing Pipeline"
echo "=================================================="
...
# Get all QTAB session directories (2 levels deep: subject/session)
echo "Scanning for QTAB sessions in: $INPUT_ROOT"
SESSIONS=($(find "$INPUT_ROOT" -mindepth 2 -maxdepth 2 -type d | sort))

# Check if we have sessions
if [ ${#SESSIONS[@]} -eq 0 ]; then
    echo "Error: No QTAB sessions found in $INPUT_ROOT"
    echo "Expected structure: $INPUT_ROOT/subject_*/session_*/"
    exit 1
fi

echo "Found ${#SESSIONS[@]} QTAB sessions to process"
...
for SESSION_DIR in "${SESSIONS[@]}"; do
    echo "Processing QTAB session: $SESSION_DIR"
