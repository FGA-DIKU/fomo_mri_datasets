...
usage() {
    ...
    echo ""
    echo "Process MGH Wild dataset with preprocessing pipeline"
    echo ""
    echo "Options:"
    echo "  -i: Input root directory containing MGH Wild subject/session folders"
    echo "  -o: Output root directory for preprocessed MGH Wild data"
    echo "  -t: Number of threads (default: 4)"
    echo "  -f: FreeSurfer home directory (default: \$FREESURFER_HOME)"
    echo "  -h: Show this help message"
    echo ""
    echo "Example:"
    echo "  $0 -i /path/to/mgh_wild_tmp -o /path/to/mgh_wild_preprocessed -t 4"
    exit 1
}
...
# Set up FreeSurfer
echo "=================================================="
echo "MGH Wild Dataset Preprocessing Pipeline"
echo "=================================================="
...
echo "Scanning for MGH Wild sessions in: $INPUT_ROOT"
SESSIONS=($(find "$INPUT_ROOT" -mindepth 2 -maxdepth 2 -type d | sort))

# Check if we have sessions
if [ ${#SESSIONS[@]} -eq 0 ]; then
    echo "Error: No MGH Wild sessions found in $INPUT_ROOT"
    echo "Expected structure: $INPUT_ROOT/subject_*/session_*/"
    exit 1
fi

echo "Found ${#SESSIONS[@]} MGH Wild sessions to process"
...
for SESSION_DIR in "${SESSIONS[@]}"; do
    echo "Processing MGH Wild session: $SESSION_DIR"
...
# Print summary
echo "=================================================="
echo "MGH Wild Processing Complete!"
echo "=================================================="
