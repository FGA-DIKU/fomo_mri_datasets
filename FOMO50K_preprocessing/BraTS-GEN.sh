...
usage() {
    ...
    echo ""
    echo "Process BraTS-GEN dataset with preprocessing pipeline"
    echo ""
    echo "Options:"
    echo "  -i: Input root directory containing BraTS-GEN subject/session folders"
    echo "  -o: Output root directory for preprocessed BraTS-GEN  data"
    echo "  -t: Number of threads (default: 4)"
    echo "  -f: FreeSurfer home directory (default: \$FREESURFER_HOME)"
    echo "  -h: Show this help message"
    echo ""
    echo "Example:"
    echo "  $0 -i /path/to/brats_gen_tmp -o /path/to/brats_gen_preprocessed -t 4"
    exit 1
}
...
# Set up FreeSurfer
echo "=================================================="
echo "BraTS-GEN Dataset Preprocessing Pipeline"
echo "=================================================="
...
# Get all BraTS-GEN session directories (2 levels deep: subject/session)
echo "Scanning for BraTS-GEN sessions in: $INPUT_ROOT"
SESSIONS=($(find "$INPUT_ROOT" -mindepth 2 -maxdepth 2 -type d | sort))

# Check if we have sessions
if [ ${#SESSIONS[@]} -eq 0 ]; then
    echo "Error: No BraTS-GEN sessions found in $INPUT_ROOT"
    echo "Expected structure: $INPUT_ROOT/subject_*/session_*/"
    exit 1
fi

echo "Found ${#SESSIONS[@]} BraTS-GEN sessions to process"
...
for SESSION_DIR in "${SESSIONS[@]}"; do
    echo "Processing BraTS-GEN session: $SESSION_DIR"
...
# Print summary
echo "=================================================="
echo "BraTS-GEN Processing Complete!"
echo "=================================================="
